        ...
        try {
            ...
            if (in_array($newStatus, ['processing', 'waiting_for_buyer', 'completed', 'cancelled'])) {
                ...
                if ($orderInfo) {
                    ...
                    $message = $emailConfig[$newStatus]['message'];
                    
                    $mail_result = send_smtp_email($to, $subject, $message);
                    
                    // Log email attempt (success or failure)
                    if ($mail_result['success']) {
                        log_action($pdo, $_SESSION['username'] ?? 'system', 'Email Notification', "{$newStatus} notification sent to {$to} for Order #{$orderId}.");
                    } else {
                        log_action($pdo, $_SESSION['username'] ?? 'system', 'Email Notification Failed', "Failed to send {$newStatus} notification to {$to} for Order #{$orderId}. Error: " . $mail_result['error']);
                    }
            }
        }
